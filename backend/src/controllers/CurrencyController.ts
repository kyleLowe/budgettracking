...
export const getCurrencyByISOCode = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const getCurrencyByCountry = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const _currencyCountry: string = req.params.currencyCountry as string;
        if (!_currencyCountry) {
            throw new NotFoundError('Currency country is required');
        }
        const currency = await CurrencyServices.getCurrencyByCountry(_currencyCountry);
        if (!currency) {
